...
using Examination.Utilities.Productivity;
...
using Examination.Utilities.NHibernate;
using Examination.Models;

			  
namespace Examination.Utilities {
	...
	public class HibernateSession {
					...
					switch (environmentOverride_testOnly ?? Config.GetEnv()) {
						case Env.local_mysql: {
								...
								try {
									...
									.Mappings(m => {
										m.FluentMappings.AddFromAssemblyOf<QuestionsModel>();
									})
		...
		private static void BuildMsSqlSchema(Configuration config, bool forceCreate = false) {
			...
			var dir = Path.Combine(Path.GetTempPath(), "Examination");
